...
class Detect(nn.Module):
    ...
    def __init__(self, nc=80, anchors=(), ch=(), inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes 类别数
        self.no = nc + 5  # number of outputs per anchor 单个Anchor匹配类别数+5个通道数
        self.nl = len(anchors)  # number of detection layers 输出3个预测特征层
        self.na = len(anchors[0]) // 2  # number of anchors 每一层预测特征图有6//2=3个Anchor
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.anchor_grid = [torch.zeros(1)] * self.nl  # init anchor grid
        self.register_buffer('anchors', torch.tensor(anchors).float().view(self.nl, -1, 2))  # shape(nl,na,2)
        #对每个输出特征图进行卷积得到特定通道数的特征图，这里输出深度为（类别数+4+1）*3
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            bs, _, ny, nx = x[i].shape  # 将输入的255层转换为 x(bs,255,20,20) to x(bs,3,20,20,85)
        ...
        #x(bs,3,20,20,85)[x1,x2,x3]其实是返回一个大小为3(3个预测特征图）的列表，表示输出的三个结果
